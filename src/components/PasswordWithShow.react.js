...
import InputWithIcon from './InputWithIcon.react';

const field_options = [
  { type: 'password', icon: 'fa fa-unlock' },
  { type: 'text', icon: 'fa fa-lock' }
]

/**
 * Password input with a show/hide icon that can be clicked
 * to reveal the password
 */
export default class PasswordWithShow extends Component {

  constructor(props) {
    super(props);
    this.onClick = this.onClick.bind(this);
    this.state = { field_option: field_options[0] }
  }

  onClick() {
    console.log('PasswordWithShow.onClick()')
    let field_option = this.state.field_option
    field_option = (field_option === field_options[0]) ? field_options[1] : field_options[0]
    this.setState({ field_option })
  }


  render() {
    const {type, icon,} = this.state.field_option
    return (
      <InputWithIcon icon={icon} type={type} onClick={e => this.onClick(e)} {...this.props} />
    );
  }
}
 ...
 * Stolen from dcc
 * 
 * https://github.com/facultyai/dash-bootstrap-components/blob/master/src/components/input/Input.js
 */
PasswordWithShow.propTypes = {
